...
INT_RSP_ROW=$base_root/tmp/integration-ordrsp-list-row.txt
...
CreateORDRSPIdList $INT_RSP_ROW $INT_RSP "$tm3"
#
## Convert to UTF-8
iconv -f UTF-8 -t ISO-8859-1 $INT_RSP_ROW > $INT_RSP
#
...
mail -s "VGR ORDRSP Copy back check at $cktm !!!" procurement.2ndline.int@ebuilder.com < $RSP_CHK
fi
#
#
fi
